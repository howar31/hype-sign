...
- Build-time `__COMMIT__` (resolved in `vite.config.ts` → `git rev-parse --short HEAD`, CI prefers `GITHUB_SHA`, dirty tree → `<sha>-dirty`, no-git → `'dev'`) is shown as a small footer in the Settings tab, followed by a row of three 32×32 glass icon buttons (`.icon-link`): GitHub repo, Ko-fi, PayPal. Each carries a `data-brand` attribute that swaps in the brand color on hover/focus (white / `#FF5E5B` / `#0070BA`). Sponsor channels = both Ko-fi (`ko-fi.com/howar31`) **and** PayPal (`donate.howar31.com`). zh tooltip uses 「贊助」 not 「捐款」 (see `~/.claude/skills/accept-donations` for the wording rule). SW update detection is **passive only**: `src/lib/swUpdate.ts` adds a `controllerchange` listener (skipped when `controller === null` to avoid first-install false positives) and exposes `useUpdateReady()`. `registerType: 'autoUpdate'` and the auto-injected `registerSW.js` are unchanged — never call `register()` / `skipWaiting()` / `update()` manually here.
