...
- `Preset` is a single-color snapshot `{ name, color }`; the same preset can be applied to either text or background via `applyPresetToText` / `applyPresetToBg`. Persist version is 3 — v1→v2 migration splits old `{textColor, bgColor}` preset pairs into two single-color presets; v2→v3 is additive (seeds `panelMode` / `floatingPos` / `mobilePanelHeight` / `floatingHeight` defaults).
