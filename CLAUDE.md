...
- Edge margin between text and screen is one setting (`margin`, 0–15 vmin). It's applied as `padding` on each display's outer wrapper so the bg color still fills the full canvas. MarqueeDisplay needs a two-div split (outer = bg + padding, inner = positioned container) because `position: absolute` is measured from the padding edge.
